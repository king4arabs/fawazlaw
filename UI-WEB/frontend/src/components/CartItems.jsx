    ...
    return (
        <div className=" w-[90%] shadow-top-darkRed py-8 pr-10 border-2 rounded-lg flex flex-col items-end justify-end">
            <div className=" flex flex-col items-end gap-5 pt-4">
              <div className={`${activeLanguage === 'ar' ? 'justify-end items-end' : ''} flex flex-col gap-3 w-[full]`}>
                <h1 className=" text-[20px] font-black  text-[#003E6F]">
                  {activeLanguage === 'ar'? title : itm.title_en}
                </h1>
                <div className={`w-full subsection flex ${activeLanguage === 'ar' ? 'flex-row' : 'flex-row-reverse'} gap-2`}>
                  <h3 className={`text-[18px] ${activeLanguage === 'ar' ? 'text-end' : ''} max-w-sm mx-auto break-words text-[#667085] font-medium pr-3`}>
                  {activeLanguage === 'ar'? content : itm.content_en}
                </h3>
            ...
            <div className={`${activeLanguage === 'ar' ? 'justify-end' : "pl-9"} items-center flex content-center w-full transition duration-[100ms] gap-1 pt-5`}>
              <span className="text-[18px] text-[#081F2F] font-bold">{t('yearly')}</span>
              <span className="text-[18px] text-[#667085] font-black">{itm.currency}</span>
